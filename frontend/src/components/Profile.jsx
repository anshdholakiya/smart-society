...
import { User, Mail, MapPin, Save, Shield, Camera } from 'lucide-react';
...
const Profile = ({ user, setUser }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    submitData.append('wing', formData.wing);
    submitData.append('flatNumber', formData.flatNumber);
    if (profilePicture) {
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-6 mb-8">
          <div className="relative group">
            <div className="w-24 h-24 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 overflow-hidden shadow-sm border-2 border-slate-100">
              {preview ? (
            ...
            <label className="absolute inset-0 bg-black/40 rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity z-10 backdrop-blur-[1px]">
              <Camera size={24} />
              <input
                type="file"
                className="hidden"
          ...
          <div>
            ...
            <span className="px-2 py-1 rounded text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100 uppercase tracking-wider mt-1 inline-block">
              {user.role}
            </span>
          </div>
        </div>
